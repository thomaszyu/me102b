 ...
 * Game screen: live table view (puck, mallet, strategy) fed from laptop over UDP
 *
 * Setup:
 ...
 *   4. Set your WiFi credentials below
 *   5. Upload, then run laptop_listener.py on your laptop
 *
 * Touch calibration:
...
const char* WIFI_PASSWORD = "happysheep342";
...
// ===================== SCREENS ====================
enum Screen : uint8_t { SCREEN_MENU, SCREEN_GAME };
Screen currentScreen = SCREEN_MENU;

// ===================== STATE ======================
enum Difficulty : uint8_t { DIFF_NONE = 0, DIFF_EASY, DIFF_MEDIUM, DIFF_HARD };
...
void drawWifiStatus(const char* text, uint16_t color) {
    tft.fillRect(0, 0, SCREEN_W, 18, COL_BG);
    tft.setTextColor(color, COL_BG);
    tft.setTextDatum(TL_DATUM);
    tft.drawString(text, 5, 3, 2);
}
...
void drawScoreBar() {
    ...
    if (strcmp(strategy, "DEFEND") == 0) stratColor = COL_DEFEND;
    else if (strcmp(strategy, "STRIKE") == 0) stratColor = TFT_CYAN;
    else if (strcmp(strategy, "WINDUP") == 0) stratColor = TFT_YELLOW;
    else if (strcmp(strategy, "PAUSED") == 0) stratColor = TFT_YELLOW;
...
void connectWiFi() {
    drawWifiStatus("Connecting...", TFT_YELLOW);
    WiFi.mode(WIFI_STA);
    ...
    if (WiFi.status() == WL_CONNECTED) {
        wifiConnected = true;
        udp.begin(UDP_PORT);
        drawWifiStatus(WiFi.localIP().toString().c_str(), TFT_GREEN);
        Serial.printf("WiFi OK: %s\n", WiFi.localIP().toString().c_str());
    } else {
        wifiConnected = false;
        drawWifiStatus("No WiFi", TFT_RED);
...
void sendStart() {
    ...
    switch (selectedDifficulty) {
        ...
        case DIFF_MEDIUM: diffStr = "medium"; break;
...
int jsonFloatArray(const char* buf, const char* key, float* outX, float* outY, int maxPts) {
    ...
    while (count < maxPts) {
        ...
        if (!next || strchr(p, ']') < next) break;  // ] before next comma = end
        p = next + 1;
    }
    return count;
}
...
void runCalibration() {
    ...
    for (int i = 0; i < 5; i++) touchCalData[i] = calData[i];
}
#endif
        ...
        if (nowConn != wifiConnected) {
            ...
            if (currentScreen == SCREEN_MENU) {
                if (wifiConnected) {
                    ...
                    drawWifiStatus("WiFi lost", TFT_RED);
                }
            }
        }
    }
